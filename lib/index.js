...
const { writeFileAsync, readFileAsync, accessAsync, unlinkAsync } =
...
const installConfigDependencies = (path) => {
	return accessAsync(join(path, 'versionist.conf.js'))
		.then(() => {
			...
			return execAsync(`npm install --only=dev`, { cwd: path }).return(false);
		})
		.catch((err) => {
...
const injectConfig = (path) => {
	return readFileAsync(join(path, 'repo.yml'), 'utf8').then((configFile) => {
			...
			return Promise.resolve(
				...
				.tap((result) => {
					...
					if (
						...
						return execAsync(`npm install --no-save ${dependencies}`, {
							cwd: path,
						});
					}
				})
