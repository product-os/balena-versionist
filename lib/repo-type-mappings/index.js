...
const exportedMappings = mappings.reduce((exportedMappings, dirName) => {
	...
	exportedMappings[type].getConfig = (options, path) => {
		...
		try {
			...
			dependencies = YAML.load(
				readFileSync(join(path, 'dependencies.yml'), 'utf8'),
			);
		} catch (err) {
			const defaultConf = readFileSync(join(dirName, CONF_FILENAME), 'utf8');
			configuration = mustache.render(defaultConf, options);
			dependencies = YAML.load(
				readFileSync(join(dirName, 'dependencies.yml'), 'utf8'),
			);
		}
